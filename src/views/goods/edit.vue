  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      <el-form ref="goods" :rules="rules" :model="goods" label-width="150px">
        ...
        <el-form-item label="市场价" prop="retailPrice">
    ...
    <el-card class="box-card">
      ...
      <el-table :data="products">
        <el-table-column property="value" label="货品规格">
          <template slot-scope="scope">
            <!-- <el-tag v-for="tag in scope.row.goods.specifications" :key="tag">{{ tag }}</el-tag> -->
            <el-tag v-for="tag in scope.row.goods.specifications" :key="tag">{{ tag }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column property="goods.productName" label="货品名称" />
        <el-table-column property="goods.price" label="货品售价" />
        <el-table-column property="goods.costPrice" label="成本价" />
        <el-table-column property="goods.number" label="货品数量" />
        <el-table-column property="goods.weight" label="货品重量" />
        <el-table-column property="brand.name" label="供应商"></el-table-column>
        <el-table-column property="goods.url" label="货品图片">
          <template slot-scope="scope">
            <img v-if="scope.row.goods.url" :src="scope.row.goods.url" width="40" />
          </template>
        </el-table-column>
        <el-table-column
          align="center"
        ...
        >
          <template slot-scope="scope">
            <!-- {{scope.row}} -->
            <el-button type="primary" size="mini" @click="handleProductShow(scope.row.goods)">设置</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- <el-table :data="products">
        ...
        <el-table-column property="goods.price" width="100" label="货品售价" />
        <el-table-column property="goods.number" width="100" label="货品数量" />
        <el-table-column property="goods.url" width="100" label="货品图片">
          <template slot-scope="scope">
            <img v-if="scope.row.goods.url" :src="scope.row.goods.url" width="40" />
          </template>
        </el-table-column>
      ...
      <el-dialog :visible.sync="productVisiable" title="设置货品">
        ...
        >
          ...
          <el-form-item label="子商品名称" prop="product_name">
            <el-input v-model="productForm.product_name" />
          </el-form-item>
          <el-form-item label="成本价" prop="cost_price">
            <el-input v-model="productForm.cost_price" />
          </el-form-item>
...
export default {
  ...
  methods: {
    init: function() {
      ...
      const goods_sn = { goods_sn: this.$route.query.id };
      ...
      detailGoods(goods_sn).then(response => {
        ...
        products.forEach((item, index) => {
          products[index].goods.specifications = JSON.parse(item.goods.specifications);
        });
        console.log(products)
        ...
        // this.categoryLevel = Object.values(this.categoryIds).join(" / ");
    ...
    //显示供应商
    supplier(row, column) {
      let data = this.brandList.filter((item, index, arr) => {
    ...
    uploadPicUrl: function(response) {
      this.goods.picUrl = response.data.allfilePath;
    },
