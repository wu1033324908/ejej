 ...
 * @Author: your name
 * @Date: 2019-08-02 10:28:55
 * @LastEditTime: 2019-08-15 17:07:45
...
<script>
import { oldUpdata, oldRead, uploadPath } from '@/api/oldExcavator'
...
export default {
  ...
  methods: {
    init() {
      ...
      _this.formData.good_code = good_code

      oldRead({ good_code })
        .then(response => {
          console.log('isArray')
          console.log('isArray :' + response.data.data.picGalleryUrlList instanceof Array)
          for (const i in response.data.data) {
            _this.$set(_this.formData, i, response.data.data[i])
            if (i === 'picGalleryUrlList') {
              // console.log(typeof JSON.parse(response.data.data[i]))
              _this.$set(_this.formData, i, JSON.parse(response.data.data[i]))
            }
          }
          // 图片初始化
