 ...
 * @Author: your name
 * @Date: 2019-08-02 10:42:20
 * @LastEditTime: 2019-08-12 17:14:38
  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      <el-form ref="employment" :rules="rules" :model="employment" label-width="150px" class="jobWanted">
        ...
        <el-form-item label="项目类型" prop="work_type">
          <el-select v-model="employment.work_type" placeholder="请选择">
            <el-option
...
<script>
import { labourWorkCreate } from '@/api/employment'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        end_time: [{ type: 'date', required: true, message: '请选择有效日期', trigger: 'change' }],
        other_desc: [{ required: true, message: '请输入其他信息', trigger: 'blur' }],
        contact: [{ required: true, message: '请输入联系人', trigger: 'blur' }],
        contact_msg: [{ required: true, message: '请输入联系方式', trigger: 'blur', validator: isValidPhone }],
        work_num: [{ required: true, message: '请输入招聘人数', trigger: 'blur' }],
        work_type: [{ required: true, message: '请输入项目类型', trigger: 'change' }]
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleCancel: function() {
      this.$router.push({
        path: '/informationPublish/employmentList'
      })
    },
    handlePublish: function(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          labourWorkCreate(finalGoods)
            .then(response => {
              this.$notify.success({
                title: '成功',
                message: '创建成功'
              })
              this.$router.push({ path: '/informationPublish/employmentList' })
            })
            .catch(response => {
              MessageBox.alert('业务错误：' + response.data.errmsg, '警告', {
                confirmButtonText: '确定',
